...
export default function FamilyWordChallengePage() {
  ...
  // تدوير الخيارات يوزّع الإجابة الصحيحة بين المواضع الثلاثة بدل ثباتها في الأول.
  const choiceShift = (stage.id + index) % question.choices.length;
  const displayChoices = [
    ...question.choices.slice(choiceShift),
    ...question.choices.slice(0, choiceShift),
  ];
  ...
  function useRemove() { const wrong=displayChoices.filter(x=>x!==question.answer&&!hidden.includes(x)).slice(0,2); setHidden(wrong); setHelps(h=>({...h,remove:false})); }
