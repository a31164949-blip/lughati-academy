...
const HOMEWORK_CACHE_KEY_PREFIX =
  "student-latest-homework-cache-v2";
...
type HomeworkCachePayload = {
  cachedAt: number;
  homework: CachedHomework | null;
};

/*
 * ننشئ مفتاح Cache مستقلًا
 * لكل فصل.
 *
 * بهذه الطريقة لا يحصل طالب
 * في الثاني أ على Cache
 * خاص بالثاني ب.
 */
function getHomeworkCacheKey(
  classroom: string
) {
  const safeClassroom =
    classroom.trim() || "unknown";

  return `${HOMEWORK_CACHE_KEY_PREFIX}:${safeClassroom}`;
}
 ...
 * sessionStorage.
 */
function readLatestHomeworkCache(
  classroom: string
): HomeworkCachePayload | null {
  try {
    const cacheKey =
      getHomeworkCacheKey(
        classroom
    ...
    console.warn(
      "تعذر قراءة ذاكرة الواجب المؤقتة:",
      error
    );
 ...
 * حفظ أحدث واجب فقط.
 *
 * لم نعد نحفظ قائمة كاملة
 * من الواجبات.
 */
function saveLatestHomeworkCache(
  classroom: string,
  homework:
    | CachedHomework
    | null
  ...
  try {
    ...
    const payload: HomeworkCachePayload =
      {
        cachedAt:
          Date.now(),
...
export default function HomeworkCheckPage() {
  ...
  async function loadLatestHomework(
    ...
    try {
      ...
      const cachedResult =
        !forceRefresh
          ? readLatestHomeworkCache(
              studentClassroom
            )
          : null;

      let latestCachedHomework:
        | CachedHomework
        | null;

      if (cachedResult) {
        latestCachedHomework =
          cachedResult.homework;
      } else {
        /*
         * نحدد الفصول المسموحة.
         *
         * طالب الثاني أ:
         * الفصلان + الثاني أ
         *
         ...
         *
         * سابقًا:
         * قراءة كل الواجبات المنشورة
         * ثم تصفيتها وترتيبها محليًا.
         *
         * الآن:
         * Firestore يعيد أحدث
         * واجب مناسب فقط.
         ...
         * "لا يوجد واجب"
         * لا تعيد القراءة
         * في كل مرة خلال 5 دقائق.
         */
        saveLatestHomeworkCache(
          studentClassroom,
          latestCachedHomework
        );
      }
  ...
  function resetHomework() {
    ...
    setMessage(
      "عاد الواجب إلى حالة: بانتظار الإنجاز."
    );
  }
...
> = {
  ...
  header: {
    ...
    boxShadow:
      "0 12px 35px rgba(28, 114, 82, 0.08)",
  },
  ...
  farisCard: {
    ...
    background:
      "linear-gradient(135deg, #16845f, #21a276)",

    color:
      "#ffffff",

    boxShadow:
      "0 15px 35px rgba(22, 132, 95, 0.18)",
  },
  ...
  homeworkCard: {
    ...
    boxShadow:
      "0 14px 40px rgba(25, 104, 76, 0.09)",
  },
  ...
  confirmButton: {
    ...
    boxShadow:
      "0 10px 22px rgba(22, 132, 95, 0.22)",
  },
